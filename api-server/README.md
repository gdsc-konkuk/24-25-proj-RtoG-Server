...
1.  서버 실행 (`api-server` 디렉토리 내에서 실행)
    ```bash
...
-   `api-server` 디렉토리 내에 `uploads` 디렉토리 생성 (또는 `config.py`의 `VIDEO_UPLOAD_DIR`에 지정된 경로) - 애플리케이션 시작 시 `VideoProcessingService`에서 자동 생성될 수 있음. 